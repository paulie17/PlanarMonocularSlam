...
struct World
{
  World(int num_samples_, float x_range_, float y_range_, float z_range_) : landmarks(num_samples_),
                                                                            x_range(x_range_),
                                                                            y_range(y_range_),
                                                                            z_range(z_range_)
  {
...
struct Camera
{
  ...
  float xi;                       // distance to unit sphere, only for omni projection

  //generates a measurement from a word and a robot position
  inline void projectPointsPinhole(PointMeasurementVector &point_measurements,
  ...
  //generates a measurement from a word and a robot position
  inline void projectPointsOmnidirectional(PointMeasurementVector &point_measurements,
                                           const WorldLandmarkVector &world_landmarks,
                                           const Eigen::Isometry2f &robot_pose) const
  {
    //world to robot, bloated to 3D
    ...
    {
      const WorldLandmark &wp = world_landmarks[i];
      const Eigen::Vector3f cp = w2c * wp.position;
      if (cp.z() < z_near)
        continue;
      if (cp.z() > z_far)
        continue;
      const Eigen::Vector3f sp = cp.normalized();
      const float iz = 1.f / (sp.z() + xi);
      Eigen::Vector2f ip = Eigen::Vector2f::Zero();
      ip.x() = K(0, 0) * (sp.x() * iz) + K(0, 2);
      ip.y() = K(1, 1) * (sp.y() * iz) + K(1, 2);
      if (ip.x() < 0 || ip.x() > width)
        continue;
      if (ip.y() < 0 || ip.y() > height)
        continue;
      // ok we accept the measurement
      PointMeasurement pm;
      pm.gt_idx = i;
...
int main(int argc, char **argv)
{
  ...
  cam.z_near = 0.1;
  cam.z_far = 5;
  // distance to unit sphere, only for omni projection
  cam.xi = 2.01;
  ...
  if (_is_pinhole)
  {
    os << "projection model: pinhole" << endl;
  }
  else
  {
    os << "projection model: omnidirectional" << endl;
    os << "xi: " << cam.xi << endl;
  }
  ...
  while (1)
  {
    ...
    if (fabs(des_rv) >= 1e-5 || fabs(des_tv) >= 1e-5 || first)
    {
      ...
      // project the point based on the settings
      if (_is_pinhole)
      {
        cam.projectPointsPinhole(point_measurements,
                                 world.landmarks,
                                 robot_pose);
      }
      else
      {
        cam.projectPointsOmnidirectional(point_measurements,
                                         world.landmarks,
                                         robot_pose);
      }
      ...
      if (motion_since_last_frame.head<2>().norm() < min_tdisp && fabs(motion_since_last_frame.z()) < min_rdisp && !first)
        continue;
